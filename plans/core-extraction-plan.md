...
今は 1 点ずつしか掴めない。エンベロープや EQ カーブでは、複数点をまとめて動かす・消すのが普通の操作になる。

- [ ] **選択の持ち方を決める。** 選択は値ではないので `points` には入れられない。Root の state か、利用者に持たせる（controlled）かを決める
- [ ] **選択の入力。** shift + クリックで追加、範囲ドラッグ（ラバーバンド）で囲む、の 2 つが要る。**範囲ドラッグは背景の空きスペースで始まるドラッグ**なので、`Background` に別の drag を張ることになる
- [ ] **修飾キーが 5.11 / 5.18 とぶつかる。** shift は既に微調整に割り当ててある。点の上での shift と背景での shift を分けるか、選択には別のキーを使うか
