...
`unitFormat('Hz', { digits: 0 })` に `step={0.1}` で値 1.6 のとき、上下キーを押すと表示は `2Hz` → `3Hz` → `4Hz` と 1 ずつ増えるのに、`onChange` が渡す値は 1.6 → 2.6 → 3.6 になる。ユーザーには「2 から 1 ずつ足した」ようにしか見えない。

**5.14 で `digit` を消しても直らなかった。** `format={(v) => v.toFixed(0)}` でも `unitFormat` の `digits` でも同じことが起きる。原因は書式の系統ではなく、表示桁と `step` が互いを知らないことなので、5.14 とは別に扱う。
