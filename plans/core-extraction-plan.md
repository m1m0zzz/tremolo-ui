...
5.11 で shift = 微調整を入れたときに表面化した。修飾キーのエントリには `step` の丸めをかけないので、**誤差を丸め戻すものが無くなり、押すたびに蓄積する。**
...
`normalized` モードでも出る（`5 → 5.1 → 5.2 → 5.300000000000001 → 5.4`）。位置に直してから戻すので出方が違うだけで、原因は同じ。

**これは 5.11 に固有の問題ではない。** リポジトリ全体で、誤差が出るたびにその場で潰す対応が散っていて、方針が無い。

| 場所 | 現在の対処 |
| --- | --- |
| `math.ts` の `stepValue` | `step` の小数桁を数えて `toFixed` する |
| `unit.ts` の `unitFormat` | 接頭辞で割った後 `toPrecision(15)` する（5.14 で追加） |
| `applyDelta` の修飾キー経路 | **何もしていない**（5.11 で追加した経路） |
| `scales.ts` の各 `Scale` | 何もしていない。`exponentialScale` は `log` / `pow` を通るのでさらに誤差が乗る |

- [ ] **どこで丸めるかを決める。** 値を作るたびに丸める（`applyDelta` / `Scale` の出口）か、表示の直前だけ丸めるか。前者は「保存されている値」が常にきれいになる代わり、利用者の `onChange` が丸めた値を受け取る
- [ ] **何桁で丸めるかを決める。** `stepValue` は `step` の小数桁、`unitFormat` は有効 15 桁と、既に 2 つの規則が併存している。`step` が無い経路（修飾キー、`normalized`、`PointsEditor` の 0〜1）では前者が使えない
- [ ] **`min` / `max` の端で厳密であること。** 丸めた結果がレンジをはみ出さないこと。`clamp` が後段にあるので実害は出にくいが、`max` ちょうどに着地できるかは別問題
- [ ] **`Scale` の往復を壊さないこと。** `scales.test.ts` は `normalize` / `denormalize` の往復を見ているので、丸めを入れるならこのテストの許容誤差と整合させる
- [ ] **利用者が丸めを外せるべきかを決める。** 内部で連続値を持ちたい利用者にとって、勝手な丸めは邪魔になりうる

**5.15（表示の桁と `step` が互いを知らない）と近いが別物。** 5.15 は「表示と値が食い違う」話で、こちらは「値そのものが汚れる」話。ただし丸めの方針を決めると 5.15 の解き方にも影響する。
