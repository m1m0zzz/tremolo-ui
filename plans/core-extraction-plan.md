...
### 5.13 NumberInput の上下キーでカーソル位置を保つ — **完了**
...
`useLayoutEffect` で行う。keydown の時点ではまだ DOM が更新されていないため。描画前に動かすので、末尾に飛んだ姿が一瞬見えることもない。

保留はテキストと一緒に持つ。**値が clamp されて動かなかった場合は何もしない**（キー入力を `preventDefault` しているのでキャレットも動いておらず、既にあるべき場所にある）。打鍵でも保留を捨てる。テキストが利用者のものになった時点で、ステップ後のテキストに対して測った位置は意味を持たない。

#### あわせて直したもの

**IME 変換中に上下キーで値が動いていた。** 変換中の上下キーは候補の選択なので、値を動かすと変換と競合し、キャレットを動かすと変換自体が壊れる。`event.nativeEvent.isComposing` で降りるようにした。

#### テスト

`src/components/NumberInput/caret.test.tsx`（9 件）。既定では末尾に飛ぶこと、桁を保つこと、連打で保ち続けること、`9.9` → `10` と `10` → `9` で小数点に追従すること、単位が付いていても数値の外へ出ないこと、clamp されたら触らないこと、打鍵で引き戻さないこと、IME を邪魔しないこと。

### 5.14 NumberInput の `units` / `digit` をやめ、`format` に一本化する — **完了**
...
- [x] `units` / `digit` / `Units` / `formatValue` / `parseValue` / `selectUnit` を削除し、`format` / `parse` だけにした。移行ガイドの Unreleased に載せてある
- [x] `units` を使っている example / story / ドキュメントを全部書き換えた。単位が `st` / `ct` / `%` のものは `prefixes: false`、ADSR は値を ms で持っているので `unitFormat('s', { base: 'm' })`
...
- [x] **名前は `unitFormat` にした。** `siUnit` は使わない。 SI でない単位（dB / % / cent / semitone）を接頭辞なしで素通しさせる以上、名前が嘘になる。`unitFormat` を推す
...
- [x] **`base` としてオプションに入れた。** 位置引数にはしない。 つまみが 3 つあり、しかも**性質が違う**ので、位置引数だと確実に混同する
...
- [x] **桁数は `digits`（小数桁）にした。** 5.13 の議論で A を採用した。`step` から既定値を導く案（B）は、`format` が `(value) => string` の純粋関数である以上 `Root` 側に暗黙の組み立てを足すことになり、「書式は 1 箇所を見れば分かる」という一本化の眼目を削るため採らない
- [x] **`prefixes: false` で宣言する形にした。** mathjs の `prefixes: 'none'` と同じ考え方。`prefixes: false` で素通し
- [x] **閾値は「小数点の前に 1 桁以上残る最大の接頭辞」にした。** UnitMath の 0.1〜1000 ではなく [1, 1000) にしたのは、説明が 1 行で済むため。両端はクランプする。丸めで桁が繰り上がっても接頭辞から溢れさせない（999.99Hz は `digits: 1` で `1.0kHz`）
- [x] **`0` は基本単位のまま出す。** 対数で接頭辞を選ぶと `0` が扱えない。基本単位で出す
- [x] **`p n µ m (なし) k M G` の 8 つに絞った。** 接頭辞を増やすほど `parse` が単位記号と見分ける手間が増える
- [x] **`{ format, parse }` の対を返す形にした。** 片方だけ差し替えられると噛み合わなくなるので、`{ format, parse }` を返して両方一度に渡す形にする

#### parse 側の落とし穴（調査で判明）

- [x] **`dB` は `prefixes: false` で扱う。** `d` は deci なので、`unitFormat('B')` にすると `-6dB` が「-6 デシ B」と解釈される。mathjs も `dB` を独立した単位として定義している
- [x] **micro は `µ` / `μ` / `u` の 3 通りを受け、`µ`（U+00B5）を出す。** d3 が使う `µ`（U+00B5 MICRO SIGN）と `μ`（U+03BC GREEK SMALL LETTER MU）は見た目が同じで別コードポイント。**parse は `µ` / `μ` / `u` の 3 通りを全部受ける**（キーボードから `µ` は打てない）。format 側でどちらを出すかも決める
- [x] **大文字小文字は潰していない。** `m` は 10⁻³、`M` は 10⁶
- [x] **単位記号を先に照合してから接頭辞を剥がす。** 単位記号自体が接頭辞と同じ文字で始まる場合（`m` = メートル に対する `mm`）に誤読しないよう、単位記号を先に照合してから残りを見る

#### 実装したもの

`unitFormat(unit, options)` を `@tremolo-ui/functions` に追加した。`{ format, parse }` の対を返し、`NumberInput.Root` にそのままスプレッドできる。

```tsx
<NumberInput.Root {...unitFormat('Hz', { digits: 2 })} value={v} onChange={setV}>
```

既定の `format` / `parse` は `NumberInput` 内に置いた。`format` は `String`、`parse` は先頭の数字を読むだけ。**`unitFormat` を既定にはしない。** 接頭辞を選ぶので、単位を指定していない入力で 1234 が `1.234k` になってしまう。

#### 踏んだもの

**接頭辞のスケールで割ると 2 進数の誤差がそのまま表示に出る。** `0.0005 / 1e-6` は `500.00000000000006` で、`digits` を渡していないと丸められないのでそのまま入力欄に出ていた。double が実際に持つ有効桁は 15 桁程度なので、`toPrecision(15)` で外を落としてから文字列にしている。

#### あわせて直した挙動

**読めないテキストを 0 として確定しなくなった。** 旧 `parseValue` は数字が無ければ 0 を返し、`commitDraft` がそれをそのまま値にしていたので、**入力欄を空にして blur すると値が 0 になっていた。** `parse` が `NaN` を返し、`commitDraft` は draft を捨てて元の値に戻す。

**ただし 5.16 の「丸めた表示が値になる」は直っていない。** `unitFormat('Hz', { digits: 0 })` で 1.6 が `2Hz` と出ている欄を編集して確定すると、値は 2 になる。draft の文字列を parse するため。フォーカス時に丸めていない値を出す（5.16）まで残る。

### 5.15 表示の桁と `step` が互いを知らない — **完了**

5.14 の調査中に判明した。表示桁と `step` の間に関係が無いため、**両者が矛盾しても何も警告されない。**

#### 決めたこと

**「表示が粗い」こと自体は防がない。「押しても何も起きない」ことだけを防ぐ。**

粗い表示は正当な設計で、実際に多い（内部は連続値、表示は丸め）。5.16 の `unformatOnFocus` でフォーカスすれば素の値が出るので、真の値に手が届かないわけでもない。ここを警告すると**正しいコードで鳴る**。

一方、**押しても表示が 1 ミリも動かない**のは常に間違い。これは 2 通りの原因で起きる。

- `keyboard` / `wheel` の量が `step` より細かい。`applyDelta` が毎回グリッドに丸め戻すので、値がそもそも動かない
- 表示が量より粗い。値は動くが `format` の出力が変わらない

#### 「`format` は任意の関数なので表示桁を機械的に知る方法が無い」— 知る必要が無かった

当初これで詰まっていたが、**桁を読む必要は無く、`format` を呼べばよい。** レンジ上の 9 点で実際に `applyDelta` を通し、値と `format` の出力が変わるかを見る。`step` と `scale` と量の相互作用は pipeline しか知らないので、`step` を読んで判断するより正確でもある。

`src/components/_util/checkSteps.ts` の `useCheckSteps`。5.6 の `useCheckPlacement` と同じ形（開発ビルドのみ、`process.env.NODE_ENV` はインラインで畳ませる、`_util/` なので typedoc に出ない）。

- [x] **9 点はレンジの内側だけ**（0.1〜0.9）。端を含めると `min` / `max` の clamp を「動かない」と誤検出する
- [x] **上下 両方向を見て、両方とも動かないときだけ警告する。** 片方だけならグリッドや端の都合で普通に起きる
- [x] **表示の警告は「レンジ全体でどこでも見えない」ときだけ。** ある値で見えないのは丸めた表示なら当たり前で、**どこでも見えないのが設定ミス**
- [x] **修飾キーのエントリも 1 つずつ確認する。** ただし修飾キーは意図的に `step` を外れるので、`applyDelta` の carve-out により普通は動く
- [x] Slider / Knob / XYPad（軸ごと）/ NumberInput に入れた。**NumberInput だけ `format` も渡す**
- [x] **NumberInput でレンジが開いているときは何も見ない。** `min` / `max` が無いと pipeline のレンジは safe-integer 範囲になり、そこを 9 分割してもサンプルとして意味を持たない
- [x] **PointsEditor.Point には入れない。** `step` も `format` も無い（値が 0〜1 の位置そのもの）ので鳴りようが無く、点の数だけ probe が走る

`site/docs/components/NumberInput/index.mdx` に「表示が粗いのは自由、見えないのは警告する」を書いた。

本番ビルドで落ちることは実測した（esbuild、`--define:process.env.NODE_ENV='"production"'`）。Slider だけを import したバンドルで 14,203 → 13,470 バイトになり、2 つのメッセージ文字列は 1 つも残らない。

#### 検証

自分のところの example / story を全部通した。**どれも鳴らなかった。** `unitFormat('s', { base: 'm', digits: 2 })` は 1 ms の押下が `1.00s` のままで危ないが、レンジのどこかで接頭辞が `ms` に切り替わるので「どこでも見えない」には当たらない。判定は保守的な側に倒れている。

- [x] `step` から表示桁の既定値を導く案（5.14 の B 案）は採らない — 一本化の眼目と衝突するため。**採らないままで済んだ**
- [x] 5.13（上下キーでカーソル位置を保つ）との関係。桁を選んで動かす操作は入れていないので、先に決める必要は生じなかった

### 5.16 NumberInput のフォーカス時に書式を外すオプション — **完了**
...
**5.14 でこの項目の重みが上がった。** `unitFormat` が接頭辞を選ぶようになったので、値 1230 に対して表示が `1.23kHz` になる場面が普通になった。以前の `units='Hz'` は数字をそのまま出していたので、表示と値の乖離は小数桁の丸めだけだった。

`selectOnFocus='number'` は先頭の数字部分だけを**選択**するので近いことはできるが、**表示自体は書式付きのまま**なので、キーで選択を外すと単位の中を編集する羽目になる。

- [x] **どの数を出すか。** 値をそのまま出す（`1.23kHz` → `1230`）。表示から数字部分だけを取る案は、単位が消えた時点で 1.23 が何なのか分からなくなり、blur すると `parse('1.23')` が 1.23 になって値が 1000 分の 1 になる
- [x] **`digits` で丸めた表示のまま編集させない。** フォーカス時に丸めていない値を出すので、`digits: 0` で 1.6 が `2Hz` と出ている欄を編集しても値が 2 にならなくなった
- [x] **prop は `InputField` に置いた。** 名前は `unformatOnFocus`
- [x] **既定値は off**
- [x] IME を壊さない

#### draft は立てない

**ここが実装の要。** フォーカス時に素の値を draft として立てる案は採らなかった。draft を立てると、**何も編集せずに blur しただけで `commitDraft` が走る**（現在は `draft === null` で早期 return している）。値がレンジ外にあるときに、ただフォーカスしただけで clamp されて `onChange` が飛ぶ。

代わりに `InputField` が自分で `focused` を持ち、表示するテキストだけを差し替える。

```tsx
const shown = unformatOnFocus && focused && !editing ? String(value) : text
```

`!editing`（= `draft === null`）が要るのは、最初の 1 打鍵から先は draft が利用者自身のテキストであり、書式の有無にかかわらずそれが正しいため。空にしても draft は `''` なので素の値には戻らない。

`Root` の context には手を入れていない。`value` / `text` / `editing` は既に載っている。

#### `selectOnFocus` との関係

**特別扱いは要らなかった。** 書式を外すとテキスト全体が数字になるので、`'number'` の正規表現が全体に一致し、結果として `'all'` と同じになる。片方を無効にしたり読み替えたりする必要がない。

ただし**選択を `useEffect` に移す必要があった。** `onFocus` の中で `setSelectionRange` を呼ぶと、テキストの差し替えより前に走るので、書式付きの古いテキストに対して範囲を取ってしまう。依存は `[focused, selectOnFocus]` だけにしてある。テキストの変化で再実行すると、利用者が打っている最中に選択が戻る。

`selectOnFocus` にはテストが 1 件も無かったので、移動にあわせて 4 件書いた。

#### キャレットの位置

`selectOnFocus` を併用しない場合、テキストを差し替えるとキャレットは末尾へ行く。**そのままにした。** 「値を打ち直す」用途では末尾が自然で、位置を保つ話は 5.13 の担当。

#### テスト

`src/components/NumberInput/focus.test.tsx`（11 件）。既定で書式が残ること、フォーカス中だけ素の値になること、**表示上の数字ではなく値が出ること**、フォーカスして離れるだけでは何も確定しないこと、丸めた表示が値にならないこと、打鍵で draft に切り替わること、フォーカス中の上下キーで素の値のままであること、`selectOnFocus` の 3 モードと書式を外したときの選択範囲。
...
5.11 で shift = 微調整を入れたときに表面化した。修飾キーのエントリには `step` の丸めをかけないので、**誤差を丸め戻すものが無くなり、押すたびに蓄積していた。**

```
step = 1、shift: ['raw', 0.1]、値 5 から shift + ↑ を連打
5 → 5.1 → 5.199999999999999 → 5.299999999999999 → 5.399999999999999 → 5.499999999999998
```

**方針: 値を作る出口で、double が実際に持てる有効 15 桁に丸める。**

`math.ts` に `toPrecision(x, significantDigits = 15)` を足し、次の 2 箇所で通す。**リポジトリ全体で値が作られるのはこの 2 箇所だけ**で、あとは全部ここを経由する。

| 場所 | 対応 |
| --- | --- |
| `applyDelta`（wheel / keyboard） | `step` の丸めの後、`clamp` の前に通す |
| `createDragValue` の `valueOf`（ドラッグ） | 同上 |
| `unit.ts` の `unitFormat` | 自前の `toPrecision(15)` をこの関数に置き換えた（規則が 1 つになる） |
| `math.ts` の `stepValue` | **そのまま。** 別の規則であることに意味がある（下記） |
| `scales.ts` の各 `Scale` | **そのまま。** 出口で丸めるので二重にする意味が無く、`normalize` はピクセル計算に流れるので丸めても得が無い |

決めたこと:

- [x] **どこで丸めるか。** 値を作る出口。表示の直前ではない。表示だけ直すと、保存されている値は汚れたままで、再入力の往復や `PointsEditor` の 0〜1 がそのまま食い違う
- [x] **何桁で丸めるか。** 有効 15 桁。double は 16 桁弱しか持てないので、**これは情報を落とす丸めではなく、元から入っていなかったものを外す操作**。`step` が無い経路（修飾キー、`normalized`、`PointsEditor` の 0〜1）でも同じ規則が使える
- [x] **`stepValue` との使い分け。** `stepValue` は「利用者が指定したグリッドに載せる」もので、値についての**判断**。`toPrecision` は「入っていなかった桁を外す」だけ。2 つの規則が併存しているのではなく、別のことをしている
- [x] **`min` / `max` の端。** 丸めは `clamp` の**前**に置いた。端に正確に着地でき、丸めた結果がレンジをはみ出すこともない
- [x] **`Scale` の往復。** `Scale` の中では丸めないので `scales.test.ts` は無傷（許容誤差は `toBeCloseTo(…, 6)`）
- [x] **利用者が外せるようにするか → しない。** 外す価値のある丸めではない。落ちるのは double の表現誤差の範囲内で、**その値は既にそれ以上の誤差を持つ浮動小数演算の結果**なので、情報は 1 ビットも失われない。全コンポーネントにオプションを足す価値は無い

実測（`toPrecision` を通した場合）:

```
5 → 5.1 → 5.2 → 5.3 → 5.4 → 5.5 → 5.6 → 5.7 …    （raw / normalized とも）
0 から 0.1 を 1000 回足す: 100 ちょうど（通さないと 99.9999999999986）
```
...
### 5.21 PointsEditor の複数選択 — **完了**

今までは 1 点ずつしか掴めなかった。エンベロープや EQ カーブでは、複数点をまとめて動かすのが普通の操作になる。

#### 決めたこと

- [x] **選択の持ち方は controlled / uncontrolled の両対応。** `selection` / `defaultSelection` / `onSelectionChange`。何も渡さなければ Root が持つ。**選択は値ではないので `points` には入れられない**（そもそも `points` という配列は Root が持っていない。点は利用者が `map` で描く）
- [x] **Root が点のレジストリを持つ。** `Point` が `id` と「今の値・min/max・onChange」を **ref で** 登録する。値は 1 フレームごとに変わるので、値をキーにしたレジストリだと毎フレーム作り直しになる
- [x] **`id` は `useId()` で自動生成、`id` prop で上書き可能。** React は `key` を子に見せないので、識別子は Point 自身が持つしかない
- [x] **追加選択は ctrl / meta。shift ではない。** shift は 5.11 / 5.18 で微調整キーになっており、両方は兼ねられない
- [x] **範囲ドラッグ（ラバーバンド）は `Container` に張る。** 背景の空きスペースで始まるドラッグ
- [x] **`createDrag` に `shouldStart` を足した。** これが無いと成立しない。点の上で押しても `Container` の drag が pointerdown を受け取り、**`onDragStart` に入る前に `setPointerCapture` を奪う**ので、点のドラッグが死ぬ。`shouldStart` は capture より前に見る
- [x] **まとめて動かすときの clamp は選択全体で 1 回。** 1 点ずつ clamp すると、端に着いた点だけ止まって選択の形が崩れる。「どの点も範囲を出ない最大の移動量」を求めて全員に同じ量を足す
- [x] **キーボードとホイールも選択全体を動かす。** フォーカスは 1 点にしか無いが、ドラッグだけ群で動いてキーだと 1 点だけ、では一貫しない
- [x] **既定は off（`selectable`）。** 選択を入れると押した/ドラッグした意味が変わる（空きスペースの押下がラバーバンドになる、点のドラッグが他も動かす）。**点ごとに意味が違うエディタ**（ADSR の 4 つのハンドルなど）には利得が無い。既定 on にすると、そういう場所で選択の枠が出て「フォーカスの枠が変なところに出ている」ように見える
- [x] **`data-selected` は outline で描く。** 既定を box-shadow にしていたが、フォーカスリングも box-shadow で描くのが普通なので**見分けが付かなくなる**
- [x] **選択中は `onChange` が同じ tick に複数回呼ばれる。** 利用者のハンドラが「render で捕まえた値」から state を作り直していると、**最後の 1 回以外が捨てられて 1 点しか動かない**。prop の doc・移行ガイド・changeset に「前の state から更新すること」を書き、story も直した。テストは `onChange` が呼ばれたことではなく**描画位置**を見る
- [x] **削除・複製は入れない。** 点の配列を持っているのは利用者で、削除が何を意味するか（配列から消す / 無効化する）はこちらからは決められない

#### ついでに直った（が、挙動が変わる）

**ドラッグが「ポインタ位置に飛ぶ」から「動かした分だけ動く」に変わった。** 点の端を掴むと、最初の move で点がカーソルの下に飛んでいた。`createDragValue` の `updateOnPointerDown` の doc は「掴んだ端でオブジェクトがずれないように」と書いてあるのに、`elementMapping` が絶対位置を返すのでずれていた。**群で動かすには差分でないと意味を持たない**ので、ここで直った。

#### 5.19 との関係

**移動は引き算と足し算なので、それだけで誤差が戻る**（`0.2 + 0.1` が `0.30000000000000004`）。テストが実際に踏んだ。`applyDeltaTo` の出口で `toPrecision` を通している。
...
`packages/react/src/components/PointsEditor/selection.test.tsx`（10 件）。既存の `index.test.tsx` は 2 件だけ期待値を書き換えた（ドラッグが差分になったため）。
