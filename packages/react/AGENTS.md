...
- 公開する props の型には独自の props だけを書く。描く要素の属性はファイル内で `type Props = XProps & Omit<ComponentPropsWithoutRef<'div'>, keyof XProps>` と合わせる。ネイティブの属性を API ページや Controls に並べないため
  ...
  - 宣言するのは型を変えるときだけ（`Root` の必須の `children`、軸ごとに受ける `aria-label`）。**独自の props が無いパートは props の型を作らず、公開もしない。** 空の interface は何も言わない名前が公開 API に増えるだけで、利用者は `ComponentProps<typeof X.Part>` で型を取れる
  - 説明したいことがあるなら JSDoc ではなくページの散文に書く
- **どのパートの `style` も `WithCSSVariables`（`src/css-variables.ts`）を通す。** props の型には書かず、`type Props = XProps & WithCSSVariables<Omit<ComponentPropsWithoutRef<'div'>, keyof XProps>, '--a' | '--b'>` と合わせる。第 2 引数はそのパートが**書き込む・読む**カスタムプロパティで、エディタの補完に出る。`style={{ '--x': … }}` を書いたときの `...style` の位置を変えたら、ここも見直すこと
