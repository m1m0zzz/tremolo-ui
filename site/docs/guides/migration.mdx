...
### Shift makes a `Knob` drag fine

Holding shift while dragging a `Knob` now moves the value a tenth as far, the
same as it does on the arrow keys. Pressing or releasing it partway through
does not disturb the value.

`dragSensitivity` rebinds it, and a bare number uses no modifier at all.

```jsx
<Knob.Root dragSensitivity={{ default: 1, alt: 0.25 }} … />
<Knob.Root dragSensitivity={1} … />
```

Only `Knob` for now. `Slider`, `XYPad` and `PointsEditor` follow the pointer
exactly, so a fine drag there is a larger change than a sensitivity — see
`plans/core-extraction-plan.md` 5.18.
