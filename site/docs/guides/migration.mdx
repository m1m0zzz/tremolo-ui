...
A selected point carries `data-selected`, and the box a drag on empty
...
### No class names, and your own on every part

**The `tremolo-` class names are gone.** A part carries what you pass to
`className` and nothing else, so the way to style anything is to give it a
class of your own:
...
The demo theme the documentation publishes is a set of CSS Modules for exactly
this — one class per part, copied into your project. See
[Styling](/docs/tutorials/styling).

**States are the attribute alone.** `[data-disabled]`, `[data-dragging]`,
`[data-selected]` and the rest are written only while they are on, in place of
`="true"` / `="false"`, and the wrappers that are not controls of their own
carry `data-disabled` / `data-readonly` where they used to carry the ARIA.
`data-vertical` became `data-orientation="horizontal|vertical"`.

**The components place their own parts.** A thumb, a mark, a point and the
selection box take themselves out of flow and centre on their value, so a
stylesheet of yours only says what they look like. `--translate` moves one off
its centre if you need that.
...
Nothing about the markup changed at the time: the same class names and the same
