...
全 P1 / P2 / P3 を判定済み。P1 は再現の有無まで確認した。対応するとしたものは全て対応済み。
...
> **状況: 対応済み** — 円弧を最大180度のSVG arcへ分割する共通path生成へ切り替え、`angleRange=360`のactive / inactive lineを2セグメントで描画するようにした。
...
> **状況: 対応済み** — `label ?? value` に変えて、空文字がそのまま描かれるようにした。あわせて `type` を `mark` / `label` に置き換え、ラベルを出さない指定は `label={null}` で表すようにした（truthy 判定でラベルの有無を決める形自体をなくすため）。
...
> **状況: 対応済み** — 他の hook と同じく `useCallbackRef` を通し、effect の依存は呼び出し側の `deps` だけにした。インラインの callback でループが張り直されないことを回帰テストで確認している。
