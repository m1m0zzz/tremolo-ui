...
> **状況: 対応済み** — 「外部の runtime 依存を持たず、内部の dom / functions パッケージだけに依存する」という説明へ英語・日本語とも書き換えた。
...
> **状況: 対応済み** — `@tremolo-ui/dom` をトップの API 一覧（en / ja）と footer に追加し、`PointsEditor` に他のコンポーネントと同じ形式の概要・import・ライブ例のページを追加した。
